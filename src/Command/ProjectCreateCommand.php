...
final class ProjectCreateCommand extends Command
{
    ...
    {
        $this
            ->setName('app:project:create')
            ->setDescription('Create an project by asking few questions')
            ->addArgument('title', InputArgument::OPTIONAL, 'The project title')
    ...
    ): int
    {
        ...
        while (null === $title) {
            $title = $io->ask('Title', $this->faker->company);
        }
        ...
        $ok = $io->confirm(
            ...
            ' <fg=blue>Title:</> <fg=white>' . $title . '</>' . PHP_EOL .
        ...
        if ($ok) {
            try {
                ...
                $this->em->flush();
                $io->success(sprintf('The project %s has been created', $title));
            } catch (\Exception $e) {
